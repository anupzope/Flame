...
# Obtain source from: git clone https://github.com/KarypisLab/GKlib.git. Edit
# the `CMakeLists.txt` file in GKlib and turn `ON` the `BUILD_SHARED_LIBS`
# options after cloning the repository. Add Compiler flag
# "-D_POSIX_C_SOURCE=199309L" in GKlib and ParMETIS to avoid error related to
# siginfo_t.
...
if [[ ${INSTALL_METIS} -eq 1 ]]; then
  ...
  make CPPFLAGS="-D_POSIX_C_SOURCE=199309L" -j4
  make install
  cd .. && echo "Directory: ${PWD}"
  cd .. && echo "Directory: ${PWD}"
...
if [[ ${INSTALL_CGNS} -eq 1 ]]; then
  ...
  cmake -DCGNS_BUILD_SHARED=ON -DCMAKE_INSTALL_PREFIX="${CGNS_IPATH}" \
    -DHDF5_DIR="${HDF5_IPATH}" -DHDF5_NEED_MPI=ON -DHDF5_NEED_ZLIB=ON  -DCGNS_ENABLE_PARALLEL=ON ..
  echo "Building CGNS..."
  make -j4
...
if [[ ${INSTALL_LOCI} -eq 1 ]]; then
  ...
  ./configure --prefix "${LOCI_IPATH}" --with-metis "${METIS_IPATH}"
  echo "Building Loci..."
  make -j4
